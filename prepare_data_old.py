...
def download_derivative(url_path, download_file):
    ...
    if not os.path.exists(download_file):
        ...
        try:
            ...
            pass
        download_derivative(url_path, download_file)
...
def create_folds(pheno, folds, seed):

    random.seed(seed)
    fold_idxs = [{'train': [], 'valid': [], 'test': []} for i in range(folds)]

    idxs = pheno['FILE_ID'].tolist()
    random.shuffle(idxs)

    train_set = np.array(idxs[:-150])
    test_set = np.array(idxs[-150:])

    cv = KFold(train_set.size, n_folds=10)
    for fold, (train_index, valid_index) in enumerate(cv):
        fold_idxs[fold]['train'] = train_set[train_index]
        fold_idxs[fold]['valid'] = train_set[valid_index]
        fold_idxs[fold]['test'] = test_set
...
if __name__ == "__main__":
    ...
    fold_idxs = create_folds(pheno, FOLDS, SEED)

    pheno_male = pheno[pheno['SEX'] == 1]
    fold_idxs_male = create_folds(pheno_male, FOLDS, SEED)

    s3_prefix = 'https://s3.amazonaws.com/fcp-indi/data/Projects/ABIDE_Initiative/Outputs/'
    download_root = './data/functionals'
    derivatives = [
        # 'cpac/filt_global/rois_cc200/{subject}_rois_cc200.1D',
        'cpac/func_minimal/{subject}_func_minimal.nii.gz',
    ]
    ...
    for derivative in derivatives:
        ...
        # print 'Overall mean:', np.mean(means), np.std(means)
        # print 'Class mean:', np.mean(means_perclass[0]), np.mean(means_perclass[1])
        # print 'Class std:', np.std(means_perclass[0]), np.std(means_perclass[1])

        # prepare_data(pheno, fold_idxs, derivatives_data, output='./data/corr/corr_1D_cv_{fold}_{datatype}.csv')
        # prepare_data(pheno_male, fold_idxs_male, derivatives_data, output='./data/corr/corr_1D_male_cv_{fold}_{datatype}.csv')
